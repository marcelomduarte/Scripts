<#
.SYNOPSIS
    Obtém informações sobre a BIOS.

.DESCRIPTION
    Este script consulta informações sobre a BIOS (Basic Input/Output System).
    
    Ele usa a classe Win32_BIOS, seguindo diretrizes do padrão CIM (Common Information Model).
...
.EXAMPLE
    # Chama a função
    $getBiosInfo = Get_BiosInfo
    $getBiosInfo
#>
Function Get_BiosInfo{
    ...
    try {
        # Consulta para obter informações sobre a BIOS
        $win32BIOS = Get-CimInstance -ClassName $classPropertys.ClassName
        
        # Exibe as informações
        $win32BIOS | Select-Object -Property $classPropertys.Property 
    }
    catch {
